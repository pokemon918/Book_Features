...
def generate_summary(
    ...
    target_words = max(150, target_words)  # Minimum 150 words
...
Write a single, flowing summary that captures all key points:"""
...
def process_book(book_folder: str) -> None:
    ...
    for i, (filename, title, text) in enumerate(chapters):
        ...
        # Save output
        output_filename = f"{filename.replace('.txt', '')}_summary.md"
...
**Summary Length:** {summary_words:,} words ({ratio:.1f}% of original)
